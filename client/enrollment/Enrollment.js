...
import {
    Card, CardHeader, CardMedia, CardActions, Typography,
    Button, makeStyles, List, ListItem, ListItemAvatar,
    ListItemSecondaryAction, ListSubheader, Avatar,
    ListItemIcon, ListItemText, Divider, Drawer, CardContent
...
const useStyles = makeStyles(theme => ({
    root: theme.mixins.gutters({
        ...
        marginTop: theme.spacing(12), marginLeft: 250
    }),
    heading: { marginBottom: theme.spacing(3), fontWeight: 200 },
    flex: { display: 'flex', marginBottom: 20 },
    card: { padding: '24px 40px 20px' },
    subheading: { margin: '10px', color: theme.palette.openTitle },
    details: { margin: '16px', },
    sub: {
    ...
    avatar: {
        color: '#9b9b9b', border: '1px solid #bdbdbd',
        background: 'none'
    },
    media: {
        height: 180, display: 'inline-block',
        width: '100%', marginLeft: '16px'
    },
    icon: { verticalAlign: 'sub' },
    category: {
        color: '#5c5c5c', fontSize: '0.9em',
        padding: '3px 5px', backgroundColor: '#dbdbdb',
        borderRadius: '0.2em', marginTop: 5
    },
    action: { margin: '8px 24px', display: 'inline-block' },
    drawer: { width: 240, flexShrink: 0, },
    drawerPaper: { width: 240, backgroundColor: '#616161' },
    content: { flexGrow: 1, padding: theme.spacing(3), },
    toolbar: theme.mixins.toolbar,
    selectedDrawer: { backgroundColor: '#e9e3df' },
    unselected: { backgroundColor: '#ffffff' },
    check: { color: '#38cc38' },
    subhead: { fontSize: '1.2em' },
    progress: {
        textAlign: 'center', color: '#dfdfdf',
        '& span': {
            color: '#fffde7', fontSize: '1.15em'
        }
    },
    para: { whiteSpace: 'pre-wrap' }
}))
...
export default function Enrollment() {
    ...
    const [enrollment, setEnrollment] = useState({
        course: { instructor: [] }, lessonStatus: []
    })
    ...
    const markComplete = () => {
        if (!enrollment.lessonStatus[values.drawer].complete) {
            ...
            complete({ enrollmentId }, { t: jwt.token }, updatedData)
                .then((data) => {
                    if (data && data.error) {
        ...
        <div className={classes.root}>
                    ...
                    <div>
                        <CardHeader
                            title={<Typography variant="h6"
                                className={classes.subheading}>Lessons</Typography>
                            }
                            subheader={
                                <Typography variant="body1"
                                    className={classes.subheading}>
                                    {enrollment.course.lessons
                                        && enrollment.course.lessons.length} lessons</Typography>}
                            action={
            ...
            {values.drawer != -1 && (<>
                <Typography variant="h5"
                    className={classes.heading}>{enrollment.course.name}</Typography>
                <Card className={classes.card}>
                    <CardHeader
